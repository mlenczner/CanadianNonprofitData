...
| [`docs/crowding-out-and-flow-through-spec.md`](docs/crowding-out-and-flow-through-spec.md) | Spec for two linked-graph analyses: does federal funding crowd out private donations, and how much federal money flows through re-granting intermediaries (with 2026-07-19 prototype results) |
...
| [`docs/grants-dashboard.html`](docs/grants-dashboard.html) | Self-contained interactive dashboard — totals, department disclosure-completeness metrics, funding chart, notable patterns (rebuild: `analysis/build_dashboard.py`) |
| [`docs/data-quality-rankings.html`](docs/data-quality-rankings.html) | Policy brief on federal disclosure quality — executive summary, recommendations, Post-December-2025 compliance, per-department evidence (rebuild: `analysis/build_quality_report.py`) |
| [`docs/index.html`](docs/index.html) | Landing page for the static site (GitHub Pages) — links to the reports, sample org/grant/evidence pages, and documentation (rebuild: `analysis/build_index.py`) |
...
| [`analysis/org_page.py`](analysis/org_page.py) | Generates a self-contained "claim and receipt" HTML profile page per organization (rebuild samples: `docs/orgs/`, spec: `docs/org-page-spec.md`). `docs/orgs/index.html` is a tombstone pointing at the live webapp's `/orgs` route (see `analysis/webapp.py`) — it no longer embeds a capped search index. |
