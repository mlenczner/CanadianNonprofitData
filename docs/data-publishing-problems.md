...
**By department** (see [dept-compliance-report.md](dept-compliance-report.md)):
...
Three departments account for virtually all non-standard values:
...
**By department — missing `federal_riding_number`** (post-Dec 2025 records only):
...
**By department — missing `recipient_business_number`** (post-Dec 2025 records only):
...
**By department — lowercase/invalid country codes:**
...
**By department — foreign/invalid codes in `recipient_province`:**
...
**By department** (descriptions under 50 characters):
...
**By department — post-Dec 2025 records** (where business number is now mandatory):
...
**By department — post-Dec 2025 records** (where riding number is now mandatory):
...
| Problem | Records Affected | Highest-Affected Department(s) | Severity |
|---|---|---|---|
...
Individual records that illustrate systemic data quality problems documented above.
...
### ESDC same-day batch publishing
...
### ESDC $1 placeholder-value awards
