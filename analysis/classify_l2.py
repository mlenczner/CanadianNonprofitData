...
def print_dry_run_report(distinct_texts, system_prompt, max_calls, backend="anthropic", out=sys.stdout, model=None):
    display_model = model or OLLAMA_MODEL
    ...
    if backend == "ollama":
        print(f"  Local model (Ollama, {display_model}) -- no API cost. No prompt caching either "
...
def write_pilot_report(con, distinct_texts, system_prompt, max_calls, out_path, seed=42, backend="anthropic", model=None):
    display_model = model or OLLAMA_MODEL
    rows = con.execute(
    ...
    if backend == "ollama":
        cost_line = (f"- Actual cost: $0.00 (local Ollama model, {display_model})."
    ...
    lines.append(f"- Backend: {backend}" + (f" ({display_model})" if backend == "ollama" else f" ({MODEL})"))
...
def main(argv=None):
    ...
    parser.add_argument("--ollama-timeout", type=int, default=OLLAMA_TIMEOUT, help="per-call HTTP timeout in seconds")
    ...
    if args.report:
        path = write_pilot_report(con, distinct_texts, system_prompt, args.max_calls, args.report,
                                   backend=args.backend, model=args.ollama_model)
        print(f"Wrote {path}")
        return

    if args.dry_run:
        print_dry_run_report(distinct_texts, system_prompt, args.max_calls, backend=args.backend, model=args.ollama_model)
        return

    if args.backend == "ollama":
        client = OllamaClient(base_url=args.ollama_base_url, num_ctx=args.ollama_num_ctx, timeout=args.ollama_timeout)
